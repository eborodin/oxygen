...
def test_scientific_notation():
    """tests that both 'e' and 'E' are parsed correctly."""
    data = StringIO(
        (
            "1.0e-1,2.0E1,3.0\n"
            "4.0e-2,5.0E-1,6.0\n"
...
def test_character_not_bytes_compatible():
    """tests exception when a character cannot be encoded as 'S'."""
    data = StringIO("–")  # == \u2013
    with pytest.raises(ValueError):
...
def test_unicode_whitespace_stripping(dtype):
    # tests that all numeric types (and bool) strip whitespace correctly
...
def test_nul_character_error(dtype):
    # tests that a \0 character is correctly recognized as an error even if
...
def test_field_growing_cases():
    # tests empty field appending/growing (each field still takes 1 character)
