"""tests the runtime usage of `numpy.typing`."""
...
def test_get_type_hints_str(name: type, tup: TypeTup) -> None:
    """tests `typing.get_type_hints` with string-representation of types."""
...
def test_keys() -> None:
    """tests that ``TYPES.keys()`` and ``numpy.typing.__all__`` are synced."""
    keys = TYPES.keys()
    ref = set(npt.__all__)
    assert keys == ref
