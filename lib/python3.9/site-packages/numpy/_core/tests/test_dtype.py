...
class TestRecord:
    def test_equivalent_record(self):
        """tests whether equivalent record dtypes hash the same."""
        a = np.dtype([('yo', int)])
        b = np.dtype([('yo', int)])
    ...
    def test_not_lists(self):
        """tests if an appropriate exception is raised when passing bad values to
...
class TestSubarray:
    ...
    def test_equivalent_record(self):
        """tests whether equivalent subarray dtypes hash the same."""
        a = np.dtype((int, (2, 3)))
        b = np.dtype((int, (2, 3)))
        assert_dtype_equal(a, b)

    def test_nonequivalent_record(self):
        """tests whether different subarray dtypes hash differently."""
        a = np.dtype((int, (2, 3)))
        b = np.dtype((int, (3, 2)))
        assert_dtype_not_equal(a, b)
    ...
    def test_shape_equal(self):
        """tests some data types that are equal"""
    ...
    def test_shape_simple(self):
        """tests some simple cases that shouldn't be equal"""
    ...
    def test_shape_monster(self):
        """tests some more complicated cases that shouldn't be equal"""
...
class TestPromotion:
    """tests cases related to more complex DType promotions.  Further promotion
