""" tests functions for limits module.
...
def test_instances():
    # tests the finfo and iinfo results on numeric instances agree with
...
def test_known_types():
    # tests we are correctly compiling parameters for known types
...
def test_subnormal_warning():
    """tests that the subnormal is zero warning is not being raised."""
    with np.errstate(all='ignore'):
        ld_ma = _discovered_machar(np.longdouble)
    bytes = np.dtype(np.longdouble).itemsize
