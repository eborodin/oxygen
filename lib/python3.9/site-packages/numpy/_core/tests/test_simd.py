...
class _SIMD_FP(_Test_Utility):
    ...
    def test_special_cases(self):
        """
        Compare Not NaN. tests intrinsics:
            npyv_notnan_##SFX
        """
...
class _SIMD_ALL(_Test_Utility):
    ...
    def test_reorder_permi128(self):
        """
        tests permuting elements for each 128-bit lane.
        npyv_permi128_##sfx
        """
        ssize = self._scalar_size()
        if ssize < 32:
    ...
    def test_arithmetic_intdiv(self):
        """
        tests integer division intrinsics:
            npyv_divisor_##sfx
            npyv_divc_##sfx
        """
        if self._is_fp():
    ...
    def test_arithmetic_reduce_sumup(self):
        """
        tests extend reduce sum intrinsics:
            npyv_sumup_##sfx
        """
        if self.sfx not in ("u8", "u16"):
            return
...
for target_name, npyv in targets.items():
    ...
    for sfxes, cls in tests_registry.items():
        for sfx in sfxes:
            ...
            tcls = type(f"tests{cls.__name__}_{simd_width}_{target_name}_{sfx}", inhr, attr)
            if skip_m:
