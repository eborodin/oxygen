...
class TestIsSubDType:
    ...
    def test_nondtype_nonscalartype(self):
        ...
        # tests the same for the correct first datatype and abstract one
...
class TestScalarTypeNames:
    ...
    def test_names_reflect_attributes(self, t):
        """ tests that names correspond to where the type is under ``np.`` """
        assert getattr(np, t.__name__) is t

    @pytest.mark.parametrize('t', numeric_types)
    def test_names_are_undersood_by_dtype(self, t):
        """ tests the dtype constructor maps names back to the type """
