...
def test_set_replace_na(i):
    # tests strings of various lengths can be set to NaN and then replaced.
    s_empty = ""
    s_short = "0123456789"
...
def test_insert_scalar(dtype, string_list):
    """tests that inserting a scalar works."""
    arr = np.array(string_list, dtype=dtype)
...
def test_sort(dtype, strings):
    """tests that sorting matches python's internal sorting."""
...
def test_argmax(strings):
    """tests that argmax/argmin matches what python calculates."""
...
def test_ufuncs_minmax(string_list, ufunc_name, func, use_out):
    """tests that the min/max ufuncs match Python builtin min/max behavior."""
...
def test_ufunc_multiply(dtype, string_list, other, other_dtype, use_out):
    """tests the two-argument ufuncs match python builtin behavior."""
    arr = np.array(string_list, dtype=dtype)
