...
class TestUfunc:
    ...
    def test_use_output_signature_for_all_arguments(self):
        # tests that providing only `dtype=` or `signature=(None, None, dtype)`
    ...
    def test_innerwt_empty(self):
        """tests generalized ufunc with zero-sized operands"""
        a = np.array([], dtype='f8')
        b = np.array([], dtype='f8')
        w = np.array([], dtype='f8')
        assert_array_equal(umt.innerwt(a, b, w), np.sum(a*b*w, axis=-1))
    ...
    def test_cross1d(self):
        """tests with fixed-sized signature."""
        a = np.eye(3)
    ...
    def test_zerosize_reduction(self):
        # tests with default dtype and object dtype
    ...
    def test_casting_out_param(self):
        # tests that it's possible to do casts on output
    ...
    def test_where_param(self):
        # tests that the where= ufunc parameter works with regular arrays
        a = np.arange(7)
        b = np.ones(7)
        c = np.zeros(7)
    ...
    def test_empty_reduction_and_identity(self):
        ...
        # tests that an empty reduction fails also if the result is empty
        arr = np.zeros((0, 0, 5))
        with pytest.raises(ValueError):
    ...
    def test_ufunc_custom_out(self):
        # tests ufunc with built in input types and custom output type
    ...
    def test_ufunc_at_boolean(self):
        # tests boolean indexing and boolean ufuncs
        a = np.arange(10)
        index = a % 2 == 0
    ...
    def test_ufunc_at_advanced(self):
        # tests empty subspace
        orig = np.arange(4)
        a = orig[:, None][:, 0:0]
        np.add.at(a, [0, 1], 3)
        assert_array_equal(orig, np.arange(4))

        # tests with swapped byte order
        ...
        # tests multiple output ufuncs raise error, gh-5665
    ...
    def test_at_not_none_signature(self):
        # tests ufuncs with non-trivial signature raise a TypeError
        a = np.ones((2, 2, 2))
        b = np.ones((1, 2, 2))
    ...
    def test_reduce_wrong_dimension_output(self, f_reduce, keepdims, out_shape):
        # tests that we're not incorrectly broadcasting dimensions.
    ...
    def test_reduce_output_does_not_broadcast_input(self):
        # tests that the output shape cannot broadcast an input dimension
...
def test_reduce_casterrors(offset):
    # tests reporting of casting errors in reductions, we test various
...
def test_object_reduce_cleanup_on_failure():
    # tests cleanup, including of the initial value (manually provided or not)
    with pytest.raises(TypeError):
...
def test_addition_reduce_negative_zero(dtype, use_initial):
    ...
    # tests various length, in case SIMD paths or chunking play a role.
