...
def test_bar_color_and_cmap_error_raises():
    ...
    # tests that providing both color and cmap raises a ValueError
    with pytest.raises(ValueError, match=msg):
...
def test_bar_invalid_color_type_error_raises():
    ...
    # tests that providing an invalid color type raises a ValueError
    with pytest.raises(ValueError, match=msg):
        df.style.bar(color=123).to_html()

    # tests that providing a color list with more than two elements raises a ValueError
    with pytest.raises(ValueError, match=msg):
