...
def test_api_date_parsing(conn, request):
    conn_name = conn
    conn = request.getfixturevalue(conn)
    # tests date parsing in read_sql
    # No Parsing
...
def test_api_date_and_index(conn, request):
    # tests case where same column appears in parse_date and index_col
    conn = request.getfixturevalue(conn)
    df = sql.read_sql_query(
        "SELECT * FROM types",
        conn,
...
def test_warning_case_insensitive_table_name(conn, request, test_frame1):
    ...
    # tests that the warning is certainly NOT triggered in a normal case.
    with tm.assert_produces_warning(None):
...
def test_sqlalchemy_type_mapping(conn, request):
    ...
    # tests Timestamp objects (no datetime64 because of timezone) (GH9085)
    df = DataFrame(
...
def test_database_uri_string(conn, request, test_frame1):
    pytest.importorskip("sqlalchemy")
    conn = request.getfixturevalue(conn)
    # tests read_sql and .to_sql method with a database URI (GH10654)
...
def test_sql_open_close(test_frame3):
    # tests if the IO in the database still work if the connection closed
...
def test_row_object_is_named_tuple(sqlite_engine):
    conn = sqlite_engine
    # GH 40682
    # tests for the is_named_tuple() function
