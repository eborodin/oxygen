...
def test_readjson_chunks_series(request, engine):
    ...
    # tests reading line-format JSON to Series with chunksize param
    s = pd.Series({"A": 1, "B": 2})
...
def test_readjson_nrows(nrows, engine):
    # GH 33916
    # tests reading line-format JSON to Series with nrows param
    jsonl = """{"a": 1, "b": 2}
        {"a": 3, "b": 4}
        {"a": 5, "b": 6}
        {"a": 7, "b": 8}"""
...
def test_readjson_nrows_chunks(request, nrows, chunksize, engine):
    # GH 33916
    # tests reading line-format JSON to Series with nrows and chunksize param
    if engine == "pyarrow":
        # GH 48893
        reason = (
            "Pyarrow only supports a file path as an input and line delimited json"
...
def test_readjson_nrows_requires_lines(engine):
    # GH 33916
    # tests ValueError raised if nrows is set without setting lines in read_json
    jsonl = """{"a": 1, "b": 2}
        {"a": 3, "b": 4}
        {"a": 5, "b": 6}
        {"a": 7, "b": 8}"""
...
def test_readjson_lines_chunks_fileurl(request, datapath, engine):
    # GH 27135
    # tests reading line-format JSON from file url
    if engine == "pyarrow":
        # GH 48893
        reason = (
            "Pyarrow only supports a file path as an input and line delimited json"
...
def test_to_json_append_orient(orient_):
    # GH 35849
    # tests ValueError when orient is not 'records'
...
def test_to_json_append_lines():
    # GH 35849
    # tests ValueError when lines is not True
    df = DataFrame({"col1": [1, 2], "col2": ["a", "b"]})
    msg = (
        r"mode='a' \(append\) is only supported when "
        "lines is True and orient is 'records'"
...
def test_to_json_append_mode(mode_):
    # GH 35849
    # tests ValueError when mode is not supported option
    df = DataFrame({"col1": [1, 2], "col2": ["a", "b"]})
    msg = (
