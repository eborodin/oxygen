...
class TestReaders:
    ...
    def test_excel_stop_iterator(self, read_ext):
        ...
        expected = DataFrame([["aaaa", "bbbbb"]], columns=["tests", "Test1"])
    ...
    def test_excel_cell_error_na(self, request, engine, read_ext):
        ...
        expected = DataFrame([[np.nan]], columns=["tests"])
        tm.assert_frame_equal(parsed, expected)
    ...
    @pytest.mark.parametrize(
        "basename,expected",
        [
            ("gh-35802", DataFrame({"COLUMN": ["tests (1)"]})),
    ...
    def test_reading_all_sheets(self, read_ext):
        # tests reading all sheet names by setting sheet_name to None,
    ...
    def test_reading_multiple_specific_sheets(self, read_ext):
        # tests reading specific sheet names by specifying a mixed list
    ...
    def test_reader_seconds(self, request, engine, read_ext):
        ...
        # tests reading times with and without milliseconds. GH5945.
        expected = DataFrame.from_dict(
            {
                "Time": [
                    time(1, 2, 3),
...
class TestExcelFileRead:
    ...
    def test_excel_passes_na(self, read_ext):
        ...
        expected = DataFrame(
            [["NA"], [1], ["NA"], [np.nan], ["rabbit"]], columns=["tests"]
        )
        tm.assert_frame_equal(parsed, expected)
        ...
        expected = DataFrame(
            [[np.nan], [1], [np.nan], [np.nan], ["rabbit"]], columns=["tests"]
        )
        tm.assert_frame_equal(parsed, expected)
        ...
        expected = DataFrame(
            [["1.#QNAN"], [1], ["nan"], [np.nan], ["rabbit"]], columns=["tests"]
        )
        tm.assert_frame_equal(parsed, expected)
    ...
    def test_excel_passes_na_filter(self, read_ext, na_filter):
        ...
        expected = DataFrame(expected, columns=["tests"])
        tm.assert_frame_equal(parsed, expected)
