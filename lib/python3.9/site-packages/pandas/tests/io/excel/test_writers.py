...
class TestRoundTrip:
    ...
    def test_creating_and_reading_multiple_sheets(self, ext):
        # see gh-9450
        #
        # tests reading multiple sheets, from a runtime
...
class TestExcelWriter:
    ...
    def test_int_types(self, np_type, path):
        # tests np.int values read come back as int
    ...
    def test_float_types(self, np_type, path):
        # tests np.float values read come back as float.
    ...
    def test_bool_types(self, path):
        # tests np.bool_ values read come back as float.
    ...
    def test_sheets(self, frame, path):
        ...
        # tests writing to separate sheets
        with ExcelWriter(path) as writer:
    ...
    def test_to_excel_interval_no_labels(self, path, using_infer_string):
        # see gh-19242
        #
        # tests writing Interval without labels.
        df = DataFrame(
    ...
    def test_to_excel_interval_labels(self, path):
        # see gh-19242
        #
        # tests writing Interval with labels.
        df = DataFrame(
            np.random.default_rng(2).integers(-10, 10, size=(20, 1)), dtype=np.int64
        )
        expected = df.copy()
    ...
    def test_to_excel_timedelta(self, path):
        ...
        #
        # tests writing timedelta to xls.
        df = DataFrame(
    ...
    # tests for Issue 11328. If column indices are integers, make
    ...
    def test_to_excel_multiindex_no_write_index(self, path):
        # tests writing and re-reading a MI without the index. GH 5616.
        ...
        # tests that it is the same as the initial frame.
    ...
    def test_comment_arg(self, path):
        # see gh-18735
        #
        # tests the comment argument functionality to pd.read_excel.
    ...
    def test_comment_default(self, path):
        # Re issue #18735
        # tests the comment argument default to pd.read_excel
    ...
    def test_comment_used(self, path):
        # see gh-18735
        #
        # tests the comment argument is working as expected when used.

        # Create file to read in.
        df = DataFrame({"A": ["one", "#one", "one"], "B": ["two", "two", "#two"]})
        df.to_excel(path, sheet_name="test_c")

        # tests read_frame_comment against manually produced expected output.
    ...
    def test_comment_empty_line(self, path):
        # Re issue #18735
        # tests that pd.read_excel ignores commented lines at the end of file

        df = DataFrame({"a": ["1", "#2"], "b": ["2", "3"]})
        df.to_excel(path, index=False)

        # tests that all-comment lines at EoF are ignored
        expected = DataFrame({"a": [1], "b": [2]})
        result = pd.read_excel(path, comment="#")
        tm.assert_frame_equal(result, expected)

    def test_datetimes(self, path):
        # tests writing and reading datetimes. For issue #9139. (xref #9185)
        unit = get_exp_unit(path)
        datetimes = [
