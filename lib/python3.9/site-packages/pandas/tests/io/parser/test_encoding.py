...
@pytest.mark.parametrize(
    ...
    [
        ...
        # tests in a data row instead of header
        ("b\n1", {"names": ["a"]}, DataFrame({"a": ["b", "1"]})),
        # tests in empty data row with skipping
        ("\n1", {"names": ["a"], "skip_blank_lines": True}, DataFrame({"a": [1]})),
        # tests in empty data row without skipping
        (
            "\n1",
...
def test_readcsv_memmap_utf8(all_parsers):
    ...
    tests correct handling of UTF-8 chars when memory_map=True and encoding is UTF-8
    """
    lines = []
    line_length = 128
    start_char = " "
