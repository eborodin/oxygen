...
class TestStata:
    ...
    def test_value_labels_old_format(self, datapath):
        # GH 19417
        #
        # tests that value_labels() returns an empty dict if the file format
...
"""
    ...
    def test_date_parsing_ignores_format_details(self, column, datapath):
        # GH 17797
        #
        # tests that display formats are ignored when determining if a numeric
...
def test_direct_read(datapath, monkeypatch):
    ...
    # tests that opening a file path doesn't buffer the file.
    ...
    # tests that we use a given fp exactly, if possible.
    with open(file_path, "rb") as fp:
        with StataReader(fp) as reader:
            assert not reader.read().empty
            assert reader._path_or_buf is fp

    # tests that we use a given BytesIO exactly, if possible.
    with open(file_path, "rb") as fp:
