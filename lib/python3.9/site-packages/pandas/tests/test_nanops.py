...
class TestNanvarFixedValues:
    ...
    def test_ground_truth(self, axis, ddof):
        # tests against values that were precomputed with Numpy.
...
class TestNanskewFixedValues:
    # xref GH 11974
    # tests data + skewness value (computed with scipy.stats.skew)
    @pytest.fixture
    def samples(self):
...
class TestNankurtFixedValues:
    # xref GH 11974
    # tests data + kurtosis value (computed with scipy.stats.kurtosis)
    @pytest.fixture
    def samples(self):
        return np.sin(np.linspace(0, 1, 200))
