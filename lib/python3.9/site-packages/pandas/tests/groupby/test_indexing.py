# tests GroupBy._positional_selector positional grouped indexing GH#42864
...
def test_list(slice_test_df, slice_test_grouped, arg, expected_rows):
    # tests lists of integers and integer valued iterables
    result = slice_test_grouped._positional_selector[arg]
    expected = slice_test_df.iloc[expected_rows]
...
def test_mix(slice_test_df, slice_test_grouped):
    # tests mixed tuple of ints and slices
...
def test_as_index(slice_test_df, arg, expected_rows):
    # tests the default as_index behaviour
...
def test_multiindex(multiindex_data):
    # tests the multiindex mentioned as the use-case in the documentation
...
def test_against_head_and_tail(arg, method, simulated):
    # tests gives the same results as grouped head and tail
    n_groups = 100
    n_rows_per_group = 30
...
def test_against_df_iloc(start, stop, step):
    # tests that a single group gives the same results as DataFrame.iloc
    n_rows = 30
...
def test_step(step):
    # tests slice with various step values
