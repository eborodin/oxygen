...
def test_ops_not_as_index(reduction_func):
    ...
    if reduction_func in ("corrwith", "nth", "ngroup"):
        pytest.skip(f"GH 5755: tests not applicable for {reduction_func}")
...
def test_wrap_aggregated_output_multindex(multiindex_dataframe_random_data):
    ...
    def aggfun(ser):
        if ser.name == ("foo", "one"):
            raise TypeError("tests error message")
        return ser.sum()

    with pytest.raises(TypeError, match="tests error message"):
...
def test_groupby_preserves_sort(sort_column, group_column):
    # tests to ensure that groupby always preserves sort order of original
...
def test_groupby_groups_in_BaseGrouper():
    # GH 26326
    # tests if DataFrame grouped with a pandas.Grouper has correct groups
