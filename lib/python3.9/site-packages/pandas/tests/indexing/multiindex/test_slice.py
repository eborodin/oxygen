...
class TestMultiIndexSlicers:
    ...
    def test_multiindex_slice_first_level(self):
        ...
        df = DataFrame(list(range(2000)), index=idx, columns=["tests"])
        df_slice = df.loc[pd.IndexSlice[:, 30:70], :]
        result = df_slice.loc["a"]
        expected = DataFrame(list(range(30, 71)), columns=["tests"], index=range(30, 71))
        tm.assert_frame_equal(result, expected)
        result = df_slice.loc["d"]
        expected = DataFrame(
            list(range(1530, 1571)), columns=["tests"], index=range(30, 71)
        )
        tm.assert_frame_equal(result, expected)
