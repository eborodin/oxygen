...
class TestSeriesReplace:
    ...
    def test_replace_nat_with_tz(self):
        # GH 11792: tests with replacing NaT in a list with tz data
    ...
    def test_replace_cascade(self, inplace):
        # tests that replaced values are not replaced again
        # GH #50778
        ser = pd.Series([1, 2, 3])
    ...
    def test_replace_change_dtype_series(self, using_infer_string):
        # GH#25797
        df = pd.DataFrame.from_dict({"tests": ["0.5", True, "0.6"]})
        warn = FutureWarning if using_infer_string else None
        with tm.assert_produces_warning(warn, match="Downcasting"):
            df["tests"] = df["tests"].replace([True], [np.nan])
        expected = pd.DataFrame.from_dict({"tests": ["0.5", np.nan, "0.6"]})
        tm.assert_frame_equal(df, expected)

        df = pd.DataFrame.from_dict({"tests": ["0.5", None, "0.6"]})
        df["tests"] = df["tests"].replace([None], [np.nan])
        tm.assert_frame_equal(df, expected)

        df = pd.DataFrame.from_dict({"tests": ["0.5", None, "0.6"]})
        df["tests"] = df["tests"].fillna(np.nan)
        tm.assert_frame_equal(df, expected)
